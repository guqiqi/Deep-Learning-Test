...
if __name__ == '__main__':
    ...
    mode, version, batch_size = list(
        map(lambda kd: kwargs.get(kd[0], kd[1]), zip(['mode', 'version', 'batch_size'], ['vgg', 'v1', 256])))
    batch_size = int(batch_size)
    model_name = mode + '_' + version
    with gm.auto_choice():
        (train_x, train_y), (test_x, test_y) = read_data('train'), read_data('test')
        train_x, test_x, train_y, test_y = reshape(train_x, False), reshape(test_x, False), np.expand_dims(train_y,
                                                                                                           -1), np.expand_dims(
            test_y, -1)
